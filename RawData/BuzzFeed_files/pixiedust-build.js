...
var dispatch = {
    ...
    push: function(data) {
        ...
        var lastData = _.last(this.queue);
    ...
    validateData: function(data) {
        ...
        for (var field in data) {
            ...
            if (!_.isNull(value) && !_.isUndefined(value) && !_.contains(acceptedTypes, dataType)) {
                if (settings.get('debug')) {
    ...
    getURL: function(events) {
        events = _.map(events, function(event) {
            return _.defaults(event, sessionVars);
        });
        var keys = _.uniq(_.flatten(_.map(events, _.keys)));
        ...
        events = _.map(events, replaceEmptyValues);

        var queryParams = _.object(_.map(keys, function(key) {
            return [key, _.pluck(events, key)];
        }));
        ...
        var params = _.map(queryParams, function(values, param) {
    ...
    setSessionVars: function(attrs) {
        ...
        sessionVars = _.extend(sessionVars, replaceEmptyValues(attrs));
    },
...
},{"./settings":4,"underscore":5}],2:[function(require,module,exports){
var Pixiedust = require('./pixiedust');

// expose pixiedust on `window`
window.Pixiedust = Pixiedust;

// `Pixiedust` still needs to be instantiated and have `setSessionVars` and `start` called
// on its instance as in the example, below:
//
// var pixiedust = new Pixiedust();
//
// pixiedust.setSessionVars({
//     'session_id': <some unique id>
// });
//
// pixiedust.start({
//     'env': <some env>,
//     'sampling_rate': <some sampling rate> // 0.0 - 1.0
// });

},{"./pixiedust":3}],3:[function(require,module,exports){
var _ = require('underscore');
var dispatch = require('./dispatch');
var settings = require('./settings');

function shouldTrack(data) {
    var options = settings.get('samplingRateOptions') || {};
    var rateForType = options[data['type']];
    var rate = _.isFinite(rateForType) ? rateForType : 1;
...
Pixiedust.prototype.push = function(data) {
    if (this.enabled && shouldTrack(data)) {
 ...
 * @param {Object}
 */
Pixiedust.prototype.start = function(opts) {
    if (this.started) {
        if (settings.get('debug')) {
    ...
    settings.set({
        ...
        'samplingRateOptions': _.clone(opts['sampling_rate_options'])
    });
...
Pixiedust.prototype.setSessionVars = dispatch.setSessionVars;
Pixiedust.prototype.flush = dispatch.sendData.bind(dispatch);

module.exports = Pixiedust;

},{"./dispatch":1,"./settings":4,"underscore":5}],4:[function(require,module,exports){
var _ = require('underscore');
...
var environmentSettings = {
    'dev': {
        'trackingURL': 'http://pixiedust-dev.buzzfeed.com',
        'debug': true
    },
    'stage': {
